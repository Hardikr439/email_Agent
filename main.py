...
# ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
async def execute_email_task(input_data: dict) -> str:
    """ Execute email sending task using EmailService """
    logger.info(f"Starting email task with input: {input_data}")
    
    # Initialize email service
    email_service = EmailService(logger=logger)
    
    # Execute the email sending task
    result = await email_service.execute_task(input_data)
...
async def start_job(data: StartJobRequest):
    ...
    try:
        ...
        logger.info(f"Received email job request to: {data.input_data['recipient_email']}")
        logger.info(f"Subject: {data.input_data['subject']}")
...
async def handle_payment_status(job_id: str, payment_id: str) -> None:
    ...
    try:
        ...
        result = await execute_email_task(jobs[job_id]["input_data"])
        print(f"Result: {result}")
        logger.info(f"Email task completed for job {job_id}")
...
async def main():
    ...
    mail = {
        "recipient_email": "himanshumain97@gmail.com",
        "subject": "Test mail",
        "body": "Used crewai masumi integration."
    }
    result = await execute_email_task(mail)
