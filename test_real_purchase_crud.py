...
async def main():
    ...
    # Configuration
    AGENT_URL = os.getenv("AGENT_API_URL", "https://crudagent-production.up.railway.app")
    ...
    AGENT_IDENTIFIER = os.getenv("AGENT_IDENTIFIER_CRUD")
    ...
    # Collect CRUD-specific input as expected by main.py
    query = input(f"{Colors.BOLD}Natural language query (default: List all collections): {Colors.END}") or "List all collections"
    database_url = input(f"{Colors.BOLD}Database URL (default: mongodb://localhost:27017): {Colors.END}") or "mongodb://localhost:27017"
    role = input(f"{Colors.BOLD}User role - employee/patient (default: employee): {Colors.END}") or "employee"
    database_name = input(f"{Colors.BOLD}Database name (default: test_db): {Colors.END}") or "test_db"
    ...
    print_info(f"CRUD Query: '{query}'")
    print_info(f"Database: {database_name} at {database_url}")
    print_info(f"User Role: {role}")
    ...
    print(f"{Colors.GREEN}✓{Colors.END} CRUD AI Agent executed your database query")
